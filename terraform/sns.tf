...
# CloudWatch Alarm for Recurring Generator Lambda Errors
resource "aws_cloudwatch_metric_alarm" "recurring_lambda_errors" {
  alarm_name          = "piggy-recurring-lambda-errors"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  statistic           = "Sum"
  threshold           = 0
  alarm_description   = "Alert when recurring generator lambda fails"
  treat_missing_data  = "notBreaching"

  dimensions = {
    FunctionName = aws_lambda_function.recurring_generator.function_name
  }
