...
func GetEntriesByMonth(monthYear string) []Entry {
	...
	query := fmt.Sprintf("https://api.toshl.com/entries?from=%s&to=%s", firstOfMonth.Format("2006-01-02"), lastOfMonth.Format("2006-01-02"))
	var entries []Entry
	json.Unmarshal([]byte(DoToshlRequest("GET", query)), &entries)
	return entries
}
